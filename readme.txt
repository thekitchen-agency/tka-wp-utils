...
1. Upload the `tka-site-utilities` folder to your `/wp-content/plugins/` directory, or upload the ZIP file directly via the WordPress admin panel.
2. Activate the plugin through the 'Plugins' menu in WordPress.
3. Navigate to the **TKA Site Utilities** admin panel in your sidebar to configure and enable settings.
...
*   Feature: Added a Global JS Interaction Delayer, allowing developers to specify custom script handles to delay entirely until scroll, click, or touch. Perfect for hiding heavy third-party plugins from Lighthouse.
*   Feature: Added a Global Asynchronous CSS Engine to easily designate stylesheet handles that should load asynchronously via the media="print" technique.
*   Feature: Introduced the Native ACF Icon Picker extension. Drop-in support for a sleek, searchable icon picker interface inside ACF fields.
*   Feature: Added Link Hover Prefetcher to intelligently preload pages when a user hovers over a link, instantly accelerating perceived page load times.
*   Fix: Refined Cross-Document Page Transitions. Prevented global site headers from flashing/fading during transitions by enforcing `mix-blend-mode: normal` and `animation: none`.
*   Fix: Resolved an issue where the new "Wipe" animations were incorrectly inheriting view transition names from the main content container.
*   Tweak: Relocated the injected Page Transitions Javascript to the footer to ensure optimal rendering performance.

= 1.12.0 =
*   Feature: Extended the Drag & Drop ordering system to natively support all Taxonomies. Administrators can now visually sort Categories, Tags, and Custom Taxonomies (like those generated by ACF) seamlessly in the WordPress backend.
*   Improvement: Enhanced the UI layout of the URL Redirect Manager to accurately mirror the unified `tka-dashboard` layout ecosystem.

= 1.11.0 =
*   Feature: Built an ultra-lightweight, blazing-fast URL Redirect Manager. Accessible as a dedicated top-level menu for all Administrators, it features a visual Repeater UI for adding 301 redirects and wildcards, routed natively at the `parse_request` level for maximum performance across Apache and Nginx servers.
